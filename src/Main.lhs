...
> import ProduceIncrementalCode (produceIncrementalParser)
...
>           action      = {-# SCC "Action" #-} (genActionTable incremental g first items2)
>           (conflictArray,(sr,rr))   = {-# SCC "Conflict" #-} (countConflicts action)
>
>           incremental = elem OptIncrementalTarget cli
>       in
...
>       let
>           parserProducer = if (elem OptIncrementalTarget cli)
>                              then (produceIncrementalParser template)
>                              else produceParser
>           outfile = parserProducer
>                       g
>                       action
>                       goto
>                       (optsToInject target cli)
...
>   = temp_dir ++ base_template ++ array_extn ++ ghc_extn ++ debug_extn
>  where
>        base_template | target == TargetIncremental = "/IncrementalTemplate"
